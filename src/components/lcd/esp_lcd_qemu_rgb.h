 ...
 * @return
 *      - ESP_ERR_INVALID_ARG: Creation failed because of invalid argument,
 * check the configuration parameter
 *      - ESP_ERR_NO_MEM: Creation failed because of the lack of free memory in
 * the heap
 *      - ESP_ERR_NOT_SUPPORTED: Creation failed because this API must only be
 * used within a QEMU virtual machine
 *      - ESP_OK: Panel created successfully
 */
esp_err_t esp_lcd_new_rgb_qemu(const esp_lcd_rgb_qemu_config_t *rgb_config,
 ...
 */
esp_err_t esp_lcd_rgb_qemu_get_frame_buffer(esp_lcd_panel_handle_t panel,
                                            void **fb);
